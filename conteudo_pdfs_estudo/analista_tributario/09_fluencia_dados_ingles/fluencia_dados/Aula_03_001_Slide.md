...
- big_data
- ingles_fiscal
arquivo_origem: Aula 03_001_Slide.txt
...
---
                                            ...
                                            Professor Renato da Costa
                                                                         Cargo atual:
                                                                         ØMinistra aulas para concursos públicos desde 1996, tendo sido
                                                                           aprovado em diversos concursos nas esferas estaduais e federais.
                                                                         ØAtualmente é servidor federal, professor de computação do IFRJ,
                                                                           tendo sido aprovado em 1º lugar em concurso de provas e títulos
                                             @prof.renatodacosta
                                                                           para o quadro efetivo.
                                                                         Formação:
                                                                         ØDoutorando em Administração pela Unigranrio; Mestre em Novas
                                                                           Tecnologias Digitais pela UniCarioca e Mestre em Educação pela
                                                                           Universidade de Jaén (Espanha), tem MBA em Gerenciamento de
                                               Renato da Costa             Projetos pela FGV, é pós-graduado em Tecnologia da Informação
                                                                           e Comunicação e pós-graduado em Docência do Ensino Superior
                                                                           sendo ambas as especializações pela Ucam, possui Licenciatura
                                                                           Plena em Informática, Matemática e Pedagogia além da
                                                                           graduação em Tecnologia em Processamento de Dados.
                                                                         Informações adicionais:
                                                                         ØÉ autor do livro "Informática pra Concursos", atualmente na 4ª
                                                                            edição, pela editora Impetus, citado como referência bibliográfica
                                                                            por diversas bancas.
Prof. Renato da Costa - TI - @prof.renatodacosta                         ØPossui certificações Cisco e Microsoft.
...
---
             ...
             sistema para computação estatística e gráfica.
...
---
         ...
         Assinale a alternativa que identifica uma linguagem e ambiente para
         computação estatística e gráficos, caracterizada pela utilização de
...
---
             ...
             O nome de uma variável (objeto) deve necessariamente
             começar com uma letra, faz distinção de letras maiúsculas e
             minúsculas, pode conter números e admite como caracteres
             especiais o underline “_” e o ponto “.”.

             OBS: Tudo no R é um objeto,                                                variáveis   podem   ser
             genericamente chamadas de objetos.
...
---
         ...
         criamos um objeto, ele fica guardado na memória do R até que se
         feche o programa. Porém, os objetos criados ficam ocultos. Para ver
...
---
              ...
              comprimentos sejam múltiplos.         vetor1       vetor2    soma
              vetor1 = c(3,7,9)                                                   3     9
...
---
              ...
              quantidade de elementos diferentes, é necessário que os
              comprimentos sejam múltiplos.         vetor1      Vetor2     soma
              vetor1 = c(3,7,9)                                                   3     9    12
              vetor2 = c(9,8,15,10,20,30)                                         7     8    15
              soma = vetor1+vetor2                                                9     15   24

              soma                                                                3     10   13
                                                        Reciclagem                7     20   27
              [1] 12 15 24 13 27 39                                               9     30   39
...
---
             ...
             do argumento from até o valor do argumento to, com salto entre
             valores consecutivos de acordo com o argumento by.
             Exemplo 1:
             seq(1,10)
...
---
    ...
    Replica o número 5 por 20                                 Cria a sequência: [1] 0 25 50 75 100 125 150
    vezes                                                     175 200
    sum(rep(5,20))                                            sum(seq(0,200,25))
...
---
             ...
             Matriz
             Uma matriz é um array bidimensional de elementos de dados do
             mesmo tipo (homogêneos).
...
---
             ...
             Função matrix()
              ...
              matrix( vetor, nrow=2, ncol=5) # Cria uma matriz distribuindo os
              dados do vetor em 2 linhas e 5 colunas
...
---
             ...
             Matrizes especiais: matriz oposta
              Dada uma matriz A, chama-se oposta de A a matriz que, adicionada
              com A, resulta na matriz nula.
...
---
             ...
             Operador %*%
           • %*% Multiplicação matricial (considera a ordem das matrizes e
             aplica as regras de álgebra linear).

           • * Multiplicação ponto a ponto (cada elemento é multiplicado
             pelo correspondente na outra matriz). Para usar * as matrizes
             devem ter exatamente o mesmo número de linhas e colunas.
...
---
               ...
               Para usar * as matrizes devem ter exatamente o mesmo número de
               linhas e colunas.
...
---
              ...
              * O determinante de uma matriz de ordem 1 é o próprio
              elemento.
              * O determinante de uma matriz quadrada de ordem 2 é o
              produto da diagonal principal subtraído do produto da diagonal
              secundária.
...
---
             ...
             Data Frames
              Um data frame é uma tabela propriamente dita e pode ser criada
              a partir dos vetores existentes através da função data.frame().
              Exemplos:
...
---
             ...
             Data Frames
              ...
              a sintaxe:
              tabela[1,]
              Para obtermos os dados referentes a terceira coluna podemos
              usar as sintaxes:
              tabela[,3]
              tabela$idades
...
---
         ...
         Dataprev – Cespe – 2023
         lista <- data.frame( id=c(10,11,12,13), nome = c('pedro’,
         'mauro’, 'maria’, 'joana’), sexo=c('M','M','F','F’), uf=
         c('PA','RJ','AL',NA), row.names=c('r1','r2','r3','r4’))
...
---
             ...
             Resumo das principais classes:
              Classe                                                            Descrição
               Vetor                                Um conjunto simples de valores de mesmo tipo.
               Matriz                    Conjunto de linhas e colunas onde cada coluna deve ter o mesmo tipo
                                                                       de dado.
              Arrays                     Arrays podem ter mais de duas dimensões. No mais, são semelhantes
                                                                    as matrizes.
               Listas                     Permite agrupar listas de diferentes objetos, que podem ter inclusive
                                                              diferentes tipos e tamanhos.
         Data Frames                               Semelhante a uma tabela de um banco de dados.
             Séries                                      Estrutura para série de dados temporais.
           Temporais
...
---
             ...
             •        var()
             •        sd()
             OBS: A base do R não oferece uma função para encontrar a moda,
             sendo necessário instalar o pacote modeest e utilizar a função mfv()
Prof. Renato da Costa - TI - @prof.renatodacosta
...
---
             ...
             Quinto desafio!!!
             Calcule a Amplitude (Intervalo) Total da coluna referente
             às idades da tabela.
...
---
                ...
                A variância é calculada a partir do quadrado dos desvios, ela é um
                número em unidade quadrada em relação à variável em questão, o
                que é inconveniente. O Desvio Padrão por sua vez é a raiz quadrada
                da variância que resulta em uma medida útil e prática.

               OBS: No contexto estatístico uma variável é, convencionalmente, o
...
---
         ...
         A construção de gráficos no software R foi revolucionada com a
         criação do pacote ggplot2, fruto de uma tese de doutorado. A
         essência desse pacote é a construção de gráficos camada por
         camada, deixando os gráficos naturalmente mais bonitos,
...
---
         ...
         TCE ES – FGV – 2023
         Observe o seguinte gráfico gerado da linguagem de programação
         R.
...
---
         ...
         Observações acerca da questão e do gráfico de dispersão:

         cex é um parâmetro que define o tamanho dos símbolos usados
         para representar os pontos no gráfico. Logo, cex=2 significa que os
         símbolos serão desenhados com o dobro do tamanho padrão.

         pch é um parâmetro que define o tipo de símbolo a ser usado. No
...
---
         ...
         MJSP – AOCP – 2020
          Um analista do MJSP necessita apresentar um gráfico para seus
         ...
         analista deve utilizar para gerar o gráfico em linguagem R.
         A) Graph.
         B) Draw.
         C) Plot.
         D) Picture.
...
---
         ...
         IBGE – IBFC – 2020
         Função da linguagem de programação R que permite fazer gráficos
...
---
        ...
        Caixa (Box): A caixa do boxplot representa o intervalo interquartil (IQR), que
        é a faixa de valores entre o primeiro quartil (Q1) e o terceiro quartil (Q3). A
        linha dentro da caixa representa a mediana dos dados. IQR=Q3-Q1
...
---
         ...
         PC BA – Idecan – 2022
          Observe o gráfico do tipo boxplot ou diagrama de caixas abaixo e
...
---
         ...
         Dataprev – Cespe – 2023
         Um gráfico muito útil para a etapa de análise de dados é o box plot,
         que ajuda a identificar a distribuição real ou padrão dos valores dos
         dados, permite visualizar mediana, quartis, valores mínimos e
         máximos, além de identificar possíveis outliers.
         (  ) CERTO           (   ) ERRADO
...
---
         ...
         TBG – Cespe – 2023
         Relativamente à figura acima, que mostra o diagrama esquemático
...
---
        ...
        Função hist()
        A função hist() cria um histograma.
        No histograma em R, a primeira classe é fechada em ambos os lados, o que
        significa que inclui tanto o limite esquerdo quanto o limite direito. A partir da
        segunda classe em diante, as classes são abertas à esquerda e fechadas à
        direita, o que significa que incluem o limite direito, mas não o limite
        esquerdo.
        Exemplo 1:
        vetor = c(10,10,13,20,25,26,27,30,33,35,40,40,41,50)
        hist(vetor)
...
---
         ...
         INEP - IBFC
         Gráfico composto por retângulos justapostos em que a base de
         cada um deles corresponde ao intervalo de classe e a sua altura à
...
---
         ...
         Pref. Ilha de Itamaracá PE – IDHTEC - 2023
         A figura a seguir é uma forma de representação gráfica adequada
         para a apresentação da distribuição de uma variável quantitativa
         contínua em estudos sobre acidentes com motocicletas em rodovias
         federais de Pernambuco
         Essa análise exploratória de dados foi realizada pelo
         superintendente Cláudio Ives do órgão responsável pela fiscalização
         de trânsito nessas rodovias. Podemos dizer que o superintendente
         utilizou uma representação gráfica denominada
...
---
         ...
         O gráfico que poderia ser utilizado para representar os dados dessa
         tabela é o:
         A) série temporal
         B) ramo-e-folha
         C) histograma
...
---
           ...
           Funções da biblioteca readr do
           pacote tidyverse
           write_csv()
           write_delim()
...
---
         ...
         código na janela R Console conforme mostra a figura apresentada,
...
---
     ...
     (   ) Certo       (    ) Errado
         ...
         #header=TRUE habilita a primeira linha do CSV como cabeçalho
         das
Prof. Renato      colunas.
             da Costa - TI - @prof.renatodacosta
...
---
         ...
         Sefin Fortaleza – Cespe – 2023
         A seguir, são apresentados dados listados no arquivo sefin.csv, que
         ...
         /home/sec/sefin.txt possui conteúdo idêntico ao arquivo
         /home/sec/sefin.csv.
Prof. Renato da Costa - TI - @prof.renatodacosta
...
---
        ...
        dados <- read.csv("DOMensalEstadoDesde1991.csv", sep = ";", dec = ",")
...
3.583333
             ...
             Vamos praticar!!!
        4) Calcule a soma dos homicídios dolosos de 2023 da 21ª Delegacia (cisp).
                  sum(dadosseg$hom_doloso[dadosseg$ano==2023& dadosseg$cisp==21]) 43

        5) Calcule a média dos homicídios dolosos de 2023 da 21ª Delegacia.
                  mean(dadosseg$hom_doloso[dadosseg$ano==2023& dadosseg$cisp==21]) 3.58333
        6) Determine o maior número de mortes mensal de 2023 da 21ª Delegacia.
                  max(dadosseg$hom_doloso[dadosseg$ano==2023& dadosseg$cisp==21])       10
...
---
        ...
        2023 da 21ª Delegacia de Polícia:
        barplot(dadosseg$hom_doloso[dadosseg$ano==2023&dadosseg$cisp==21])
...
---
             ...
             Vamos praticar!!!
        Vamos criar um histograma a partir dos homicídios dolosos de 2023 da 21ª
        DP:
...
---
        ...
        hist(dadosseg$hom_doloso[dadosseg$ano==2023], breaks=2)

          O parâmetro breaks=2 sugere
          duas classes ao R, não significa
           que será exatamente plotado
                     em duas.
...
---
             ...
             Regressão linear
        No exemplo a variável independente ou preditora é a temperatura e a
        variável dependente, prevista ou explicada é o consumo.
...
---
             ...
             Função lm()
        A função lm() é utilizada para ajustar modelos de regressão linear.


        Sintaxe:
        lm(variavel_dependente~variável_independente, dataset)
        Exemplo:
        modelo<-lm(consumo~temp, dados)


        # operador ~ é usado para separar a variável dependente das variáveis
        independentes no modelo de regressão.
...
---
             ...
             Função predict()
        A função predict() em R é usada para fazer previsões com base em um
        modelo estatístico ajustado. Ela é comumente usada após ajustar um modelo
        de regressão, como o modelo de regressão linear criado pela função lm().
        Exemplo 1:
        predict(modelo, data.frame(temperatura=50))


        Exemplo 2:
        predict(modelo, data.frame(temperatura=c(31,50))
...
---
     ...
     “Um algoritmo é uma norma executável para estabelecer um certo efeito
     desejado, que na prática será geralmente a obtenção de uma solução a um
     certo tipo de problema.” (GUIMARÃES & LAGES, pág. 5)
...
---
        ...
        Entre as estruturas de repetição, também conhecidas como laços, estão
        disponíveis as mais usuais:
        - repeat
        - while
        - for
Prof. Renato da Costa - TI - @prof.renatodacosta
...
---
        ...
        Estruturas de Repetição (laços)
        •     break é o comando para interromper um laço.
        •     next faz com que um laço seja executado uma próxima vez, permitindo
              abreviar uma repetição.
...
---
        ...
        Na estrutura REPEAT, o código é interrompido com o comando BREAK
        comumente associado a uma condição até que esta seja verdadeira.
        Exemplo:
        i=0
        repeat{
        If (i==10) {break} else{
...
---
   ...
   Exemplo 1:                                        Esse código em R define uma função chamada `fatorial` que
   fatorial <- function(n) {                         calcula o fatorial de um número inteiro não negativo `n`.
                                                     Aqui está como a função funciona:
      if (n == 0) {                                  1. Primeiro, verifica se `n` é igual a 0. Se for, a função retorna 1,
                                                         porque o fatorial de 0 é 1.
        return(1)                                    2. Se `n` não for igual a 0, a função calcula o fatorial de `n`
                                                     multiplicando `n` pelo fatorial de `n - 1`. Isso é feito chamando
      } else {
                                                     recursivamente a função `fatorial` com `n - 1` como argumento.
        return(n * fatorial(n - 1))                  Essencialmente, a função utiliza recursão para calcular o fatorial
                                                     de `n` até que `n` seja igual a 0, caso em que retorna 1.
      }}
Prof. Renato da Costa - TI - @prof.renatodacosta
...
---
             ...
             CRAN
        O CRAN (Comprehensive R Archive Network) é um repositório de software
        para a linguagem de programação R. Ele contém uma vasta coleção de
        pacotes, documentação e outras ferramentas relacionadas ao R, que são
        disponibilizadas gratuitamente para a comunidade de usuários do R em todo
        o mundo. O CRAN é mantido e gerenciado por voluntários e fornece acesso
        fácil aos pacotes R, permitindo que os usuários instalem e atualizem pacotes
        diretamente do repositório usando o R ou ferramentas associadas.
...
---
        ...
        tidyverse - utilizado para manipulação, exploração e visualização de dados.
...
---
        ...
        Amaral, F. (2016). Introdução à Ciência de Dados: Mineração de Dados e BIG
        DATA. Rio de Janeiro: Alta Books.
