---
cargo: Analista-Tributário da Receita Federal do Brasil (ATRFB)
banca: FGV
disciplina: Contabilidade Geral e Demonstrações Contábeis
tags:
- contabilidade
- cpc
- balanco_patrimonial
- dre
- dfc
- dupont
arquivo_origem: Aula 03_001_Slide.txt
tipo_material: Curso Teórico Base
aula_numero: '03'
titulo_aula: Contabilidade Geral
artigos_citados:
- Art. 176
---

# Contabilidade Geral

Contabilidade Geral

                                          Prof. Gilmar Possati
                                          Instagram: @profgilmarpossati
                                          Telegram: @canalcontabilizando
                                          Canal You Tube: Contabilizando

     


---

    CONTAS

                                     Prof. Gilmar Possati

---

                                                                       elementos patrimoniais
                               Nomes que qualificam os
                                                                        elementos de resultado

                                                                                   devedores
                                 Quantificam por meio de saldos
                                                                                    credores


  Contas
Contábeis
(conceito)                As entidades possuem certa liberdade na nomenclatura das contas

                           É vedada a utilização de designações genéricas                        Art. 176, §2º, Lei 6.404/76

                                      diversas contas     contas correntes

@profgilmarpossati   @canalcontabilizando
                                                   


---

     Questão de Prova

     Existem elementos patrimoniais e de resultados de uma mesma
     natureza que podem ser agrupados em um único item, denominado
     conta, que passa a representar tais elementos.

@profgilmarpossati   @canalcontabilizando
                                            


---

                                   Baseado no método das
                                                                                    A todo débito corresponde um ou
                                      partidas dobradas
                                                                                    mais créditos, sendo que o total de
                                 (método veneziano/digráfico)
                                                                                    débito é igual ao total de créditos.

 Mecanismo de
                                     Soma dos débitos = soma dos créditos
débito e crédito

                                Para cada débito existe um crédito correspondente

 @profgilmarpossati   @canalcontabilizando
                                                   


---

     Questão de Prova

     Para vários créditos, é desnecessário que existam vários débitos,
     basta que os valores das aplicações e das fontes de recursos sejam
     exatamente iguais. Tal regra é denominada partida simples.

@profgilmarpossati   @canalcontabilizando
                                            


---

                                                                                                   DÉBITO
                          1. Ativos e Despesas            Natureza DEVEDORA
                                                                                                   CRÉDITO

Natureza das                   2. Passivos, Receitas e PL                 Natureza CREDORA             CRÉDITO
  Contas
                                                                                                        DÉBITO

                                                                         Natureza CREDORA            DÉBITO      CRÉDITO
                       3. Contas redutoras        Ativo
                          (retificadoras)
                                                  Passivo e PL
                                                                               Natureza DEVEDORA      CRÉDITO     DÉBITO
  @profgilmarpossati    @canalcontabilizando
                                                     


---

     Natureza das Contas

  “os leigos em Contabilidade, geralmente, são levados a pensar que
  débito significa algo desfavorável, e crédito algo favorável. Na
  realidade, isto não ocorre, pois tais denominações são, hoje,
  simplesmente convenções contábeis, com uma função específica em
  cada conta”. Prof. Iudícibus
@profgilmarpossati   @canalcontabilizando
                                            


---

                          SISBB - Sistema de Informações Banco do Brasil
                      19/10/20X1          Auto-Atendimento          08:04:10
                                     Extrato de conta corrente

                Agência: xxxxx       Conta: xxxxx
                Cliente: Xxxxxx
                 =============================================================
                   Data               Histórico                       Valor

                     30/09/X1                Saldo Anterior                        0,00 C
                     01/10/X1                Recebimento de Proventos              15.000,00 C
                     01/10/X1                Pagamento de Título                   1.400,00 D
                     04/10/X1                Pagamento conta telefone              44,78 D
                     04/10/X1                Pagamento Energia Elétrica            110,35 D
                     05/10/X1                Transferido para Poupança             1.200,00 D
                     06/10/X1                Transferência on line                 801,27 C
                     06/10/X1                TED                                   8.500,00 D
                     06/10/X1                Pagamento de Título                   574,72 D
                     06/10/X1                Pagamento de Impostos                 208,55 D
                     12/10/X1                Compra com Cartão                     13,65 D
                     12/10/X1                Saque no TAA                          1.900,00 D
                     19/10/X1                S A L D O                             1.849,22 C
@profgilmarpossati    @canalcontabilizando
                                              


---

                             Entrada no Ativo → Bens e Direitos → Debitar
                              Saída no Ativo → Bens e Direitos → Creditar
                             Entrada no Passivo → Obrigações → Creditar
                               Saída no Passivo → Obrigações → Debitar
                          Despesas → apropriação (reconhecimento) → Debitar
                          Receitas → apropriação (reconhecimento) → Creditar

@profgilmarpossati   @canalcontabilizando
                                            


---

   a) Compra à vista de veículos no valor de R$ 30.000,00.
                                 Entrada de bens no Ativo → Debitar a conta “Veículos”
                                 Saída de bens no Ativo → Creditar a conta “Caixa”


   D – Veículos
   C – Caixa .000,00


 b) Compra a prazo de material de consumo no valor de R$ 10.000,00.
                             Entrada de bens no Ativo → Debitar a conta “Material de Consumo”
                             Entrada de obrigações no Passivo → Creditar a conta “Fornecedores”

   D – Material de Consumo
   C – Fornecedores .000,00

@profgilmarpossati   @canalcontabilizando
                                                 


---

   c) Pagamento de fornecedores no valor de R$ 10.000,00.
                           Saída de bens no Ativo → Creditar a conta “Caixa”
                           Saída de obrigações no Passivo → Debitar a conta “Fornecedores”

     D – Fornecedores
     C – Caixa .000,00

d) Pagamento com cheque de despesas administrativas no valor de R$
5.000,00.  Apropriação de despesas → Debitar a conta “Despesas Administrativas”
                         Saída de bens no Ativo → Creditar a conta “Bancos”

     D – Despesas Administrativas
     C – Bancos .000,00

@profgilmarpossati   @canalcontabilizando
                                               


---

   e) Recebimento de juros ativos (receitas financeiras) no valor de R$
   10.000,00.   Entrada de bens → Debitar a conta “Caixa”
                                Reconhecimento de Receita → Creditar a conta “Juros Ativos”

@profgilmarpossati   @canalcontabilizando
                                               


---

 Contas estáveis x instáveis
 Uma conta é estável credora quando sempre assume saldo credor.
 Ex: Capital Social.
 Uma conta é estável devedora quando sempre assume saldo devedor.
 Ex: Caixa.
 Uma conta é instável quando assumir saldo credor ou devedor,
 dependendo da situação.
 Ex. Lucros e Prejuízos Acumulados.

@profgilmarpossati   @canalcontabilizando
                                            


---

     Questão de Prova

     As contas de natureza devedora são as representativas de fontes de
     recursos. Servem como exemplo contas relacionadas às
     disponibilidades, como caixa e bancos, utilizadas como fonte de
     recursos para eventual investimento das entidades.

@profgilmarpossati   @canalcontabilizando
                                            


---

 Questão de Prova
 Aumentam o saldo das contas de Patrimônio Líquido, Ativo e Passivo,
 os lançamentos nelas efetuados que representam, respectivamente:
 a) Débito, Crédito e Débito.
 b) Crédito, Crédito e Crédito.
 c) Crédito, Débito e Crédito.
 d) Crédito, Crédito e Débito.
 e) Débito, Débito e Crédito.

@profgilmarpossati   @canalcontabilizando
                                            


---

                                                       patrimoniais                    Ativo, Passivo e PL
                          Representar os itens
                             e elementos
                                                         de resultado                    Receitas e Despesas


                                                                                       Data
  Função e                                                         RAZÃO               Histórico
Estrutura das                                                                          Débito
   Contas
                                                                                       Crédito
                       Elementos essenciais das
                               contas                                                   Saldo


                                                                                                Conta X
                                                                    Razonete
                                                                                          Débito    Crédito

  @profgilmarpossati   @canalcontabilizando
                                                  


---

     Função e Estrutura das Contas
 Estrutura Padrão

                                           Nome da Conta
                     Data           Histórico Débito   Crédito                    Saldo

 Para fins de simplificação, usamos o razonete:

@profgilmarpossati    @canalcontabilizando
                                             


---

     Função e Estrutura das Contas
                                                    Título da Conta
                                               Débito            Crédito

                                        Saldo Devedor                           Saldo Credor


                                     Caixa                                                            Veículos
            Saldo Inicial
                                10.000 10.000                                     Saldo Inicial   15.000
                                 5.000                                                            25.000
                                 5.000                                                            40.000

@profgilmarpossati      @canalcontabilizando
                                                    


---

     Plano de Contas
 ✓ Conjunto de contas, diretrizes e normas que disciplina as tarefas do setor de
   contabilidade.

                     Objetivo = uniformizar os registros contábeis de uma entidade.


 ✓ Deve ser flexível.

@profgilmarpossati     @canalcontabilizando
                                              


---

     Plano de Contas


                      Código da Conta                          Descrição da Conta
                      1                       Ativo
                      1.1                     Ativo Circulante
                      1.1.1                   Disponível
                      1.1.1.1                 Caixa
                      1.2                     Ativo Não Circulante

@profgilmarpossati   @canalcontabilizando
                                            


---

     Plano de Contas


   Elenco de Contas                          Manual de Contas                           Modelos de
                                                                                      Demonstrações
relação        de       contas              guia para o contabilista                   Padronizadas
(intitulação) e respectivos                 registrar    uniformemente
códigos      utilizados   pela              todos os eventos envolvidos            Modelos      dos     demais
entidade                                    na gestão do patrimônio da             quadros, livros auxiliares,
                                            entidade.                              fichas, relatórios, gráficos,
                                                                                   etc    que     devem      ser
                                                                                   elaborados pelo setor de
                                                                                   contabilidade

@profgilmarpossati   @canalcontabilizando
                                              


---

                       As contas contábeis podem ser                         Individualização do registro
                         subdivididas em subcontas
                                                                                              Exemplo:
                                                                                              Conta “Bancos”
                                                                                              1.1.2 Bancos

                                                                                              1.1.2.1 Banco do Brasil
                                                                                              1.1.2.2 Caixa Econômica Federal
                                                                                              (...)
 Contas e
Subcontas

                                                                  Consolidam contas a ela vinculadas
                                            Sintéticas

                     Classificação

                                            Analíticas
                                                                          Utilizadas para controle e acompanhamentos específicos

@profgilmarpossati   @canalcontabilizando
                                                    


---

   Teoria das Contas
                                              TEORIA DAS CONTAS
                     PERSONALISTA                MATERIALISTA                      PATRIMONIALISTA
                      Proprietários              Contas Integrais                  Contas Patrimoniais
            PL, Receitas e Despesas                  Ativo e Passivo                Ativo, Passivo, PL


            Agentes Consignatários            Contas Diferenciais PL,              Contas de Resultado
                                               Receitas e Despesas
        Pessoa      a    quem      o                                               Receitas e Despesas
        proprietário confia a guarda
        dos bens da empresa.


         Agentes Correspondentes
        Pessoas que não pertencem
        à entidade. São os direitos e
         as obrigações da empresa.
@profgilmarpossati     @canalcontabilizando
                                              


---

Questão de Prova
A evolução do pensamento científico em Contabilidade foi marcada pela
contribuição de diversos pensadores que culminaram no desenvolvimento das
chamadas Teorias das Contas, as quais subdividem as rubricas contábeis em
grandes grupos. A respeito desse assunto, podemos afirmar que

a) a teoria personalista subdivide as contas em Contas do Proprietário e Contas
de Agentes Consignatários.

b) a teoria materialista subdivide as contas em Contas Materiais e Contas de
Resultado.


@profgilmarpossati   @canalcontabilizando
                                            


---

  c) a teoria patrimonialista subdivide as contas em Contas Patrimoniais e Contas
  Diferenciais.

  d) a teoria personalista subdivide as contas em Contas do Proprietário e Contas
  de Agentes Correspondentes.

  e) a teoria materialista subdivide as contas em Contas Integrais e Contas
  Diferenciais.

@profgilmarpossati   @canalcontabilizando
                                            


---

Instagram: @profgilmarpossati
Telegram: @canalcontabilizando
Canal You Tube: Contabilizando

---
