---
cargo: Analista-Tributário da Receita Federal do Brasil (ATRFB)
banca: FGV
disciplina: Contabilidade Geral e Demonstrações Contábeis
tags:
- contabilidade
- cpc
- balanco_patrimonial
- dre
- dfc
- dupont
arquivo_origem: Aula 05_001_Slide.txt
tipo_material: Curso Teórico Base
aula_numero: '05'
titulo_aula: Contabilidade Geral
---

# Contabilidade Geral

Contabilidade Geral

                                          Prof. Gilmar Possati
                                          Instagram: @profgilmarpossati
                                          Telegram: @canalcontabilizando
                                          Canal You Tube: @contabilizando

     


---

ESCRITURAÇÃO

                                      Prof. Gilmar Possati

 


---

Escrituração
                                             Técnica utilizada p/ o registro dos fatos contábeis
                          Conceito
                                             Realizada por meio de lançamentos em livros

                                  Deve ser mantida em registros                        Legislação societária
                                   permanentes, obedecendo
                                                                                       Princípios de Contabilidade

Escrituração                     Deve observar métodos ou critérios UNIFORMES
                                 Regulamentação: ITG 2000 (R1)

                                                      Empresário
                          Obrigatoriedade
                                                      Sociedade Empresária
                                                      Deve    levantar    anualmente    o   balanço
                                                      patrimonial e de “resultado econômico” (DRE)

                                                                            Produtor Rural
                                                     Exceção
                                                                            Pequeno Empresário (MEI)
@profgilmarpossati   @canalcontabilizando
                                                  


---

Questão de Prova


  A contabilidade utiliza-se de registros por meio de lançamentos de
  partidas dobradas.

@profgilmarpossati   @canalcontabilizando
                                            


---

Livros de Escrituração
                                                                           Exigido pelo código civil
                                               obrigatório                                  Pequeno empresário (MEI)
                                                                           Exceção
                                               principal
                      É um livro                                         Registra TODOS os fatos contábeis
                                               comum                       Para TODAS empresas
                                               cronológico                Fatos contábeis registrados em ordem cronológica
Livro DIÁRIO


                          permite compreender a sequência de acontecimentos ocorridos na empresa

                                NÃO pode ser substituída pela escrituração obrigatória do LIVRO RAZÃO

 @profgilmarpossati     @canalcontabilizando
                                                       


---

Livros de Escrituração
                           Possibilita controlar o movimento de todas contas de forma separada.


                                                       Termo de abertura;
Livro RAZÃO
                                Deve conter             Termo de encerramento;

                                                       Assinatura do contabilista e do responsável pela empresa.


                          Demonstra a movimentação analítica das contas escrituradas no Diário e evidenciadas no
                          balanço patrimonial.
                                                                                                    conforme legislação societária
                                                                               Regra geral
                                                facultativo
                                                obrigatório                   Para entidades que apuram IR pelo lucro real
                      É um livro
                                                principal                    Registra TODOS os fatos contábeis
                                                sistemático
                                                                              Fatos contábeis registrados por tipo de contas
 @profgilmarpossati      @canalcontabilizando
                                                                                                           Bancos, duplicatas a receber,
                                                                                fornecedores.


---

Livros de Escrituração


                     registra cronologicamente todos os pagamentos e recebimentos


Livro CAIXA             Não existe determinação legal/normativa sobre sua forma de escrituração


                     Obrigatório para empresas enquadradas no regime tributário Simples Nacional

@profgilmarpossati   @canalcontabilizando
                                               


---

Livros de Escrituração

                                               Registro de ações nominativas e partes beneficiárias nominativas

                                              Transferência de ações nominativas e partes beneficiárias nominativas


Livros Obrigatórios para as                  Atas das Assembleias Gerais, reuniões do Conselho de
Sociedades Anônimas (SAs)                            Administração e reuniões da Diretoria.

                                             Presença dos acionistas

                                              Atas e pareceres do Conselho Fiscal

 @profgilmarpossati   @canalcontabilizando
                                                      


---

Questão de Prova

 Os principais livros utilizados pela contabilidade são o livro diário e
 o livro caixa. No primeiro, são escriturados os fatos decorrentes da
 atividade da companhia, explicados no histórico, e, no segundo, são
 feitos os registros em cada conta do sistema contábil.

@profgilmarpossati   @canalcontabilizando
                                            


---

 Formalidades da Escrituração
                          Refere-se à forma de apresentação material do livro

                          Finalidade: dificultar a adulteração do livro
                                                                                     ser encadernados
                                                 Não digitais                       ter folhas numeradas
                                                                                    conter termo de abertura e de fechamento
                       Em livros contábeis                                                                 da entidade (responsável)
Extrínsecas
                       obrigatórios devem                                       conter assinaturas
                                                                                                           do contador (habilitado)

                                                                        Devem ser assinados digitalmente (entidade e contador)
                                                 Digitais
                                                                       Autenticados em registro público ou entidade
                                                                       competente (quando exigível por legislação específica)

                        Em caso de não observância = invalida TODO o livro

  @profgilmarpossati   @canalcontabilizando
                                                    


---

 Formalidades da Escrituração
                          Relacionadas com o lançamento contábil

                         Finalidade: resguardar a fidedignidade dos fatos ocorridos

                                                   Idioma e moeda nacional
                                                   Em forma contábil
                      A escrituração deve
                         ser executada              Ordem cronológica de dia, mês e ano
Intrínsecas                                        Com ausência de espaços em branco, entrelinhas,
                                                   borrões, rasuras e emendas

                                    Com base em documentos de origem externa e interna


                                             Na falta de documentos, em elementos que comprovem ou
                                             evidenciem fatos contábeis.


                        Em caso de não observância = invalida APENAS o registro

 @profgilmarpossati   @canalcontabilizando
                                                    


---

Lançamentos Contábeis
 Os elementos essenciais do lançamento contábil no livro Diário são:
 - local e data;
 - conta(s) debitada(s);
 - conta(s) creditada(s);
 - histórico;
 - valor.

@profgilmarpossati   @canalcontabilizando
                                            


---

  Lançamentos Contábeis

@profgilmarpossati   @canalcontabilizando
                                            


---

Fórmulas de Lançamentos

@profgilmarpossati   @canalcontabilizando
                                            


---

Fórmulas de Lançamentos

1ª fórmula: Compra à vista de veículo no valor de R$ 20.000,00.


                 Débito – Veículos
                 Crédito – Caixa ... 20.000,00


                            Ativo → Natureza Devedora → Entrada de Bens (Veículo) → Debita
                            Ativo → Natureza Devedora → Saída de Bens (Caixa) → Credita

@profgilmarpossati   @canalcontabilizando
                                               


---

  Fórmulas de Lançamentos
  2ª fórmula: Recebimento de duplicatas no valor de R$ 30.000,00, com juros de
  10%.

  Débito – Caixa ... 33.000,00
  Crédito – Duplicatas a Receber ... 30.000,00
  Crédito – Juros Ativos ... 3.000,00

                     Ativo → Natureza Devedora → Entrada de Bens (Dinheiro) → Debita
                     Ativo → Natureza Devedora → Saída de Direitos (Duplicatas a Receber) → Credita
                     Receita → Natureza Credora → Aumento de Receitas (Juros Ativos) → Credita

@profgilmarpossati   @canalcontabilizando
                                              


---

Fórmulas de Lançamentos
3ª fórmula: Pagamentos de duplicatas no valor de R$ 20.000,00, com juros de 10%.


  Débito – Duplicatas a Pagar ... 20.000,00
  Débito – Juros Passivos... 2.000,00
  Crédito – Caixa ... 22.000,00

                     Passivo → Natureza Credora → Saída de Obrigações (Duplicatas a Pagar) → Debita
                     Despesas → Natureza Devedora → Aumento de Despesas (Juros Passivos) → Debita
                     Ativo → Natureza Devedora → Saída de Bens (Dinheiro) → Credita

@profgilmarpossati   @canalcontabilizando
                                              


---

 Fórmulas de Lançamentos
 4ª fórmula: Venda à vista de mercadorias no valor de R$ 60.000,00 ao custo de
 40.000,00.

  Débito – Caixa ... 60.000,00
  Débito – Custo da Mercadoria Vendida... 40.000,00
  Crédito – Receita de Vendas ... 60.000,00
  Crédito – Estoque de Mercadorias ... 40.000,00

                                       Ativo → Natureza Devedora → Entrada de Bens (Dinheiro) → Debita
                                       Receitas → Natureza Credora → Aumento de Receitas (Vendas) → Credita
                                       Ativo → Natureza Devedora → Saída de Bens (Mercadorias) → Credita
                                       Despesa → Natureza Devedora → Aumento de Despesas (CMV) → Debita

@profgilmarpossati   @canalcontabilizando
                                                   


---

Questão de Prova

O lançamento de segunda fórmula corresponde a mais de um débito
e apenas um crédito, enquanto o de terceira fórmula corresponde a
um débito e diversos créditos.

@profgilmarpossati   @canalcontabilizando
                                            


---

Erros de Escrituração e Retificação

                                            Título

                                             Valor
                                            Inversão
Erros de escrituração
     (principais)                           Duplo registro
                                            Omissão
                                            Histórico incorreto

@profgilmarpossati   @canalcontabilizando
                                            


---

  O estorno consiste em lançamento inverso àquele feito erroneamente,
  anulando-o integralmente. Em regra, a correção envolve dois
  lançamentos, o de estorno e o que corretamente registra a transação.

                      Exemplo: Registro errado de compra à vista de veículos

                     Lançamento errado                                            Retificação

                                                                      D – Fornecedores
  D – Veículos                                                        C – Veículos ... 20.000
  C – Fornecedores ... 20.000
                                                                      D – Veículos
                                                                      C – Caixa ... 20.000
@profgilmarpossati   @canalcontabilizando
                                            


---

  A transferência consiste no estorno parcial. É aquele que promove a
  regularização de conta indevidamente debitada ou creditada, por
  meio da transposição do registro para a conta adequada.

                     Exemplo: Registro errado de compra à vista de veículos

                     Lançamento errado                                           Retificação

  D – Veículos                                                    D – Fornecedores
  C – Fornecedores ... 20.000                                     C – Caixa ... 20.000


@profgilmarpossati   @canalcontabilizando
                                            


---

   A complementação consiste em complementar, aumentando ou
   reduzindo o valor anteriormente registrado. Assim, repete-se o
   lançamento errado e registra-se a diferença de valor.

              Exemplo: Registro errado de compra à vista de veículos no valor de
                        R$ 15.000,00, sendo o correto R$ 20.000,00.
                     Lançamento errado                                            Retificação

  D – Veículos                                                        D – Veículos
  C – Caixa ... 15.000                                                C – Caixa ... 5.000


@profgilmarpossati   @canalcontabilizando
                                            


---

 Questão de Prova

 A retificação de lançamento é o processo técnico de correção de
 registro que tenha sido realizado com erro na escrituração contábil
 da entidade e pode ser feita por meio de estorno, transferência ou
 complementação.

@profgilmarpossati   @canalcontabilizando
                                            


---

Questão de Prova

O lançamento suplementar serve para estornar o registro original
que foi lançado com valor inferior ao devido.

@profgilmarpossati   @canalcontabilizando
                                            


---

Escrituração de Operações Típicas
  Operação 1: Subscrição de capital com integralização em dinheiro no
  valor de R$100.000.

  Efeitos do fato contábil → aumento na conta caixa (+A) e aumento na conta capital social (+ PL).

  Classificação → fato modificativo aumentativo (+A; +PL)

     D – Caixa
     C – Capital Social ... 100.000

@profgilmarpossati   @canalcontabilizando
                                            


---

  Escrituração de Operações Típicas
  Operação 2: Depósito bancário no valor de R$40.000.

Efeitos do fato contábil → saída na conta caixa (- A) e entrada de valor na conta bancos (+ A).

Classificação → fato permutativo (-A; +A)

          D – Bancos
          C – Caixa... 40.000


@profgilmarpossati   @canalcontabilizando
                                            


---

  Escrituração de Operações Típicas
  Operação 3: Compra de mercadorias a prazo por R$20.000,
  aceitando duplicatas.

Efeitos do fato contábil → entrada de valor na conta estoque (+A) e surgimento de uma obrigação (+P).

Classificação → fato permutativo (+A; +P)

    D – Estoques
    C – Duplicatas a pagar... 20.000


@profgilmarpossati   @canalcontabilizando
                                            


---

 Escrituração de Operações Típicas
  Operação 4: Aplicação financeira no valor de R$10.000.

  Momento 1 → aplicação
  Ocorre somente uma troca de valores entre componentes patrimoniais (+A, -A).
  Momento 2 → apropriação* dos rendimentos
  Há o reconhecimento da aplicação financeira pelo rendimento obtido, ou seja, houve um aumento de PL
  em virtude de uma receita financeira (+A, +PL).
  Momento 3 → resgate
  É o inverso da aplicação, ou seja, a empresa resgata o valor da aplicação e retorna para a conta no
  banco (+A,-A). Nesse caso, somam-se os rendimentos apropriados, se houver.
  *Apropriar significa reconhecer a ocorrência do fato gerador de uma receita ou despesa.

@profgilmarpossati       @canalcontabilizando
                                                                


---

 Escrituração de Operações Típicas
  Operação 4: Aplicação financeira no valor de R$10.000.

  Efeitos do fato contábil → diminuição de valor da conta Bancos (-A) e aumento de valor da conta Aplicação
  Financeira (+A).

  Classificação → fato permutativo (-A; +A)

    D – Aplicação Financeira
    C – Banco... 10.000


@profgilmarpossati   @canalcontabilizando
                                              


---

 Escrituração de Operações Típicas
 Operação 5: Obtenção de empréstimo bancário no valor de
 R$3.000.

  Momento 1 → obtenção do empréstimo
  Ocorre somente uma troca de valores entre componentes patrimoniais (+A, +P).
  Momento 2 → apropriação dos juros (provisão dos encargos financeiros)
  Há o reconhecimento da apropriação dos encargos financeiros incidentes sobre a operação, ou
  seja, houve uma diminuição do PL em virtude de uma despesa financeira e o aumento do passivo
  (+P, -PL).
  Momento 3 → pagamento (quitação da dívida)
  Corresponde à saída de valor da conta bancos para quitar a obrigação assumida (-A, -P).


@profgilmarpossati   @canalcontabilizando
                                            


---

 Escrituração de Operações Típicas
 Operação 5: Obtenção de empréstimo bancário no valor de
 R$3.000.
 Efeitos do fato contábil → aumento de valor da conta Bancos (+A) e aumento de valor da conta
 Empréstimos Bancários (+P).

 Classificação → fato permutativo (+A; +P)

    D – Banco
    C – Empréstimos Bancários ... 3.000


@profgilmarpossati   @canalcontabilizando
                                             


---

 Escrituração de Operações Típicas
 Operação 6: Venda de mercadorias à vista por R$10.000, com lucro
 de 20% sobre o preço de venda.

 Momento 1 → Reconhecimento da venda
 Corresponde ao valor de entrada, vale dizer, o preço pelo qual a mercadoria foi vendida.
 Momento 2 → Reconhecimento do custo
 Corresponde ao valor de saída, ou seja, ao custo da mercadoria que foi baixada do estoque da
 entidade.

@profgilmarpossati   @canalcontabilizando
                                            


---

 Escrituração de Operações Típicas
 Operação 6: Venda de mercadorias à vista por R$10.000, com lucro
 de 20% sobre o preço de venda.
 Efeitos do fato contábil → entrada de valor na conta Caixa (+A); saída de valor do estoque (-A); e aumento
 no PL, decorrente da receita com vendas (+PL).

 Classificação → trata-se de um fato misto aumentativo (+A; -A; +PL)

    D – Caixa
    C – Receita c/ Vendas ... 10.000


@profgilmarpossati   @canalcontabilizando
                                            


---

 Escrituração de Operações Típicas
 Operação 6: Venda de mercadorias à vista por R$10.000, com lucro
 de 20% sobre o preço de venda.

 D – CMV
 C – Estoque... 8.000

@profgilmarpossati   @canalcontabilizando
                                            


---

   Escrituração de Operações Típicas
 Operação 7: Apropriação da folha de pagamento no valor de
 R$2.000.

Efeitos do fato contábil → reconhecimento de uma despesa, diminuindo o PL (-PL) e surgimento de uma
obrigação (+P).

Classificação → fato modificativo diminutivo (+P; -PL)

       D – Despesas c/ Salários
       C – Salários a pagar ... 2.000

@profgilmarpossati   @canalcontabilizando
                                            


---

 Escrituração de Operações Típicas
 Operação 8: Apropriação de rendimentos financeiros no valor de
 R$1.000.

 Efeitos do fato contábil → aumento no saldo da aplicação financeira (+A) e reconhecimento de uma
 receita, aumentando o PL da empresa (+PL).

 Classificação → fato modificativo aumentativo (+A; +PL)

     D – Aplicação Financeira
     C – Rendimentos Financeiros... 1.000

@profgilmarpossati   @canalcontabilizando
                                            


---

Escrituração de Operações Típicas
 Operação 9: Pagamento de duplicata no valor de R$4.000, em
 dinheiro, com juros de 10%.
 Efeitos do fato contábil → baixa na obrigação (duplicatas a pagar) (-P), diminuição da conta caixa (-A) e
 reconhecimento de uma despesa (juros passivos), diminuindo o PL (-PL).
Classificação → trata-se de um fato misto diminutivo (-A; -P; -PL)
                                                                                 Valor da duplicata: 4.000,00
                                                                                 Despesa de juros (10%): 400,00
                                                                                 Valor desembolsado: 4.400,00


D – Duplicatas a pagar ... 4.000
D – Juros Passivos ... 400
C – Caixa ... 4.400


@profgilmarpossati   @canalcontabilizando
                                            


---

  Escrituração de Operações Típicas
  Operação 10: Pagamento, em dinheiro, de despesas administrativas
  no valor de R$500.
  Efeitos do fato contábil → saída de valor da conta caixa (-A) e reconhecimento de uma despesa, diminuindo
  o PL (-PL).
  Classificação → fato modificativo diminutivo (-A; -PL)

         D – Despesas Administrativas
         C – Caixa ... 500

@profgilmarpossati   @canalcontabilizando
                                             


---

  Escrituração de Operações Típicas
  Operação 11: Pagamento dos salários provisionados em dinheiro.

  Efeitos do fato contábil → saída de valor da conta caixa (-A) e a baixa de uma obrigação (-P).

  Classificação → fato permutativo (-A; -P)

         D – Salários a pagar
         C – Caixa ... 2.000

@profgilmarpossati   @canalcontabilizando
                                              


---

  Escrituração de Operações Típicas
   Operação 12: Compra de veículo para uso por R$20.000, sendo 10%
   à vista, em dinheiro, e o restante a prazo.
   Efeitos do fato contábil → entrada de valor na conta Veículos (+A); saída de valor da conta Caixa (-A); e
   entrada de valor na conta Financiamentos a Pagar (+P).

   Classificação → fato permutativo (+A; -A; +P)

D – Veículos ... 20.000
C – Financiamentos a pagar ... 18.000
C – Caixa ... 2.000

 @profgilmarpossati   @canalcontabilizando
                                             


---

 Escrituração de Operações Típicas
   Operação 13: Pagamento, em cheque, de duplicata no valor de
   R$6.000, com desconto de 10%.
   Efeitos do fato contábil → baixa de obrigação (duplicata a pagar) (-P); saída de valor da conta Bancos (-A);
   e reconhecimento de uma receita (descontos obtidos), aumentando o PL (+PL).

   Classificação → fato misto aumentativo (-A; -P; +PL)                           Valor da duplicata: 6.000,00
                                                                                  Desconto obtido (10%): 600,00
                                                                                  Valor desembolsado: 5.400,00

D – Duplicatas a pagar ... 6.000
C – Banco ... 5.400
C – Descontos Obtidos ... 600

 @profgilmarpossati   @canalcontabilizando
                                             


---

 Escrituração de Operações Típicas
  Operação 14: Desconto de R$ 5.000 em duplicatas a vencer no
  prazo de 30 dias, sob a taxa de juros de 10% ao mês.

  Efeitos do fato contábil → aumento do saldo da conta Bancos (+A); aumento do saldo da conta Encargos
  Financeiros a Transcorrer (a vencer) (-P); e aumento do saldo da conta Duplicatas Descontadas (+P).

  Classificação → fato permutativo (+A; -P; +P)                                       Valor da duplicata: 5.000,00
                                                                                      Encargos Financeiros (10%): 500,00
                                                                                      Valor Ativado na conta Bancos: 4.500,00

D – Banco ... 4.500
D – Encargos Financeiros a Transcorrer ... 500
C – Duplicatas Descontadas... 5.000

 @profgilmarpossati   @canalcontabilizando
                                                 


---

Escrituração de Operações Típicas
  No vencimento das duplicatas temos três hipóteses:

  1ª Hipótese → O cliente paga as duplicatas.
  D – Duplicatas Descontadas
  C – Duplicatas a Receber

  2ª Hipótese → O cliente não paga as duplicatas.
  D – Duplicatas Descontadas
  C – Bancos

  3ª Hipótese → O cliente paga apenas uma parte do valor das duplicatas.
  D – Duplicatas Descontadas
  C – Duplicatas a Receber
  C – Bancos
@profgilmarpossati   @canalcontabilizando
                                            


---

Questão de Prova

 Em 01/06/X9, a empresa Dara efetuou o desconto de duplicatas no valor de R$
 30.000,00, cujo vencimento era 31/07/X9, à taxa de juros de 2,5% a.m. (juros
 simples). As despesas cobradas pelo banco foram de R$ 500,00. Sabendo que
 no dia 31/07/X9 o cliente não pagou a duplicata, a empresa, nesta data, debitou:
 a) Duplicatas Descontadas e creditou Disponível, no valor de R$ 28.000,00.
 b) Duplicatas Descontadas e creditou Clientes, no valor de R$ 30.000,00.
 c) Perdas com Clientes e creditou Clientes, no valor de R$ 28.000,00.
 d) Duplicatas Descontadas e creditou Disponível, no valor de R$ 30.000,00.
 e) Disponível e creditou Duplicatas Descontadas no valor de R$ 28.000,00.

@profgilmarpossati   @canalcontabilizando
                                            


---

BALANCETE DE VERIFICAÇÃO

                                            Prof. Gilmar Possati

       


---

Definição

      ▪ demonstrativo auxiliar;
      ▪ não obrigatório;
      ▪ levantado para fins operacionais;
      ▪ composto por todas as contas com os seus respectivos saldos,
        extraídos do Livro Razão.

@profgilmarpossati   @canalcontabilizando
                                            


---

Objetivo


    ▪ verificar se o método das partidas dobradas foi obedecido ao
      longo do processo de escrituração dos fatos contábeis

@profgilmarpossati   @canalcontabilizando
                                            


---

Elementos Mínimos


   a) identificação da entidade;
   b) data a que se refere;
   c) abrangência;
   d) identificação das contas e respectivos grupos;
   e) indicação dos saldos das contas (devedores ou credores);
   f) soma dos saldos devedores e credores.

@profgilmarpossati   @canalcontabilizando
                                            


---

Tipos

   Podem ser de 4 tipos, de acordo com o número de colunas: 2, 4, 6 ou 8.


                                            Balancete de 2 colunas

                           Contas                   Saldo Devedor                   Saldo Credor
                                                       Coluna 1                       Coluna 2

                Total

@profgilmarpossati   @canalcontabilizando
                                               


---

Tipos

   Podem ser de 4 tipos, de acordo com o número de colunas: 2, 4, 6 ou 8.


                                                 Balancete de 4 colunas
                                                 Movimentos
                             Contas                             Saldo Devedor            Saldo Credor
                                              Devedor Credor
                                              Coluna 1 Coluna 2    Coluna 3               Coluna 4

                     Total

@profgilmarpossati     @canalcontabilizando
                                                    


---

Tipos

   Podem ser de 4 tipos, de acordo com o número de colunas: 2, 4, 6 ou 8.


                                              Balancete de 6 colunas
                                         Saldo Anterior                   Movimentos         Saldo Atual
                     Contas
                                       Devedor    Credor              Devedor   Credor    Devedor Credor
                                       Coluna 1 Coluna 2              Coluna 3 Coluna 4   Coluna 5 Coluna 6

          Total

@profgilmarpossati     @canalcontabilizando
                                                  


---

Tipos

   Podem ser de 4 tipos, de acordo com o número de colunas: 2, 4, 6 ou 8.


                                                 Balancete de 8 colunas
                                                                 Movimentos
                            Saldo Anterior                                                 Saldo Atual
            Contas                                                      Saldos
                                                    D         C
                              D             C                         D        C          D          C
                              C1            C2     C3         C4     C5        C6         C7         C8
        Total

@profgilmarpossati   @canalcontabilizando
                                                     


---

Resumindo...


           Tipo
                                                                  Estrutura
         (colunas)
           Duas         saldo devedor (D); saldo credor (C)
          Quatro        movimentos (D/C); saldo devedor (D); saldo credor (C)
            Seis        saldo anterior (D/C), movimento (D/C); saldo atual (D/C)
           Oito         saldo anterior (D/C); movimento (D/C) e Saldos (D/C); saldo atual (D/C)

@profgilmarpossati   @canalcontabilizando
                                             


---

Atenção!


 O fato de o somatório dos saldos devedores ser igual ao somatório dos saldos
 credores não significa que a escrituração está 100% correta.

 Existem erros que não podem ser detectados pelo levantamento do balancete:
 omissão de registro, erro de título, determinados registros em duplicidade,
 entre outros.

@profgilmarpossati   @canalcontabilizando
                                            


---

Sintético x Analítico

    O balancete pode ser sintético ou analítico.

    SINTÉTICO: relaciona apenas as contas principais ou de primeiro grau
    (exemplo: conta “Bancos”).

    ANALÍTICO: relaciona as subcontas de segundo, terceiro e demais graus
    (exemplo: conta “Banco do Brasil S.A”).

@profgilmarpossati   @canalcontabilizando
                                            


---

  Questão de Prova

  O balancete de verificação pode ser levantado em modelos que
  vão de um mínimo de duas colunas a um máximo de seis colunas.

@profgilmarpossati   @canalcontabilizando
                                            


---

Questão de Prova

    Cabe ao contador escolher a quantidade de colunas a ser adotada
    no balancete de verificação, desde que respeitado o limite máximo
    de seis colunas.

@profgilmarpossati   @canalcontabilizando
                                            


---

Questão de Prova

     O balancete de verificação será elaborado em até oito colunas, na
     qual são apresentados os saldos iniciais, o movimento, os saldos
     do período e os saldos finais.

@profgilmarpossati   @canalcontabilizando
                                            


---

Questão de Prova

     A diferença entre os modelos de balancete de 6 e de 8 colunas
     corresponde aos saldos devedores e credores que dizem respeito
     ao movimento do período, constantes no segundo modelo e
     ausentes no primeiro.

@profgilmarpossati   @canalcontabilizando
                                            


---

Questão de Prova

     O balancete de verificação é um relatório de conferência que
     corresponde a uma simples listagem dos saldos devedores e
     credores, cujos totais devem ser iguais.

@profgilmarpossati   @canalcontabilizando
                                            


---

Questão de Prova

     Quando a soma dos saldos credores for igual à soma dos saldos
     devedores no balancete de verificação, não haverá nenhum erro
     nos lançamentos contábeis do período.

@profgilmarpossati   @canalcontabilizando
                                            


---

Questão de Prova

     O balancete analítico relaciona as subcontas de segundo, terceiro
     e outros graus e o balancete sintético relaciona apenas as contas
     principais ou de primeiro grau.

@profgilmarpossati   @canalcontabilizando
                                            


---

        Prof. Possati
      @profgilmarpossati
 Canal You Tube Contabilizando
Telegram: @canalcontabilizando

          


---
