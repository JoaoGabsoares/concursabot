...
router.post('/add-single', (req, res) => {
    ...
    try {
        let deck = db.prepare('SELECT id FROM flashcard_decks WHERE subject = ? AND user_id = ? AND (career_id = ? OR career_id IS NULL) LIMIT 1').get(subject, userId, careerId);
        let deckId;

        if (deck) {
            deckId = deck.id;
        } else {
            const info = db.prepare('INSERT INTO flashcard_decks (topic, subject, user_id, career_id) VALUES (?, ?, ?, ?)').run(topic, subject, userId, careerId);
            deckId = info.lastInsertRowid;
        }

        const cardStmt = db.prepare('INSERT INTO flashcards (deck_id, front, back, user_id) VALUES (?, ?, ?, ?)');
        const cardInfo = cardStmt.run(deckId, front, back, userId);
    ...
    try {
        ...
        if (careerId) {
            stmt = db.prepare(`
                ...
                WHERE d.user_id = ? AND (d.career_id = ? OR d.career_id IS NULL)
                GROUP BY d.id
            `);
...
router.get('/decks/:id', (req, res) => {
    try {
        const userId = getAuthenticatedUserId(req);
        const deck = db.prepare('SELECT * FROM flashcard_decks WHERE id = ? AND user_id = ?').get(req.params.id, userId);
        if (!deck) return res.status(404).json({ error: 'Baralho não encontrado' });
...
router.get('/review', (req, res) => {
    try {
        const userId = getAuthenticatedUserId(req);
        const stmt = db.prepare("SELECT * FROM flashcards WHERE next_review <= datetime('now') AND user_id = ? LIMIT 50");
        res.json(stmt.all(userId));
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
router.post('/review', (req, res) => {
    ...
    try {
        const card = db.prepare('SELECT * FROM flashcards WHERE id = ? AND (user_id = ? OR user_id IS NULL)').get(cardId, userId);
...
router.delete('/decks/:id', (req, res) => {
    try {
        const userId = getAuthenticatedUserId(req);
        const stmt = db.prepare('DELETE FROM flashcard_decks WHERE id = ? AND user_id = ?');
